...
var (
	...
	methodReviewOnly = method{"review only",
		"Ariane reads this once merged, so it cannot be exercised beforehand"}
)
		...
		case workflowPath.MatchString(p):
			...
			if err == nil {
				...
				if err == nil && triggers[pullRequestTrigger] {
					// GitHub loads it from the merge ref, so this pull
					// request already runs the proposed version.
					continue
				}
			}
			if err != nil {
			...
			unvalidated = append(unvalidated, finding{p, triggers[dispatchTrigger]})
		}
	}
	return unvalidated
}
...
func (s step) instructions(c prContext) []string {
	...
	case methodTempTrigger.id:
		return []string{
			"     mirror this branch to cilium/cilium, add a temporary",
			fmt.Sprintf("     'pull_request' trigger to %s, and open a test", workflow),
			"     pull request from it; revert the trigger before merge",
		}
	default:
...
func renderNotice(steps []step, unvalidated []string, rearm bool, c prContext) string {
	...
	var block []string
	for i, s := range steps {
		heading := fmt.Sprintf("%2d. %s", i+1, sanitize.Replace(s.workflow))
		if s.workflow == "" {
			heading = fmt.Sprintf("%2d. (no workflow found that consumes these)", i+1)
		}
