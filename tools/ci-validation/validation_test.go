...
func TestUnvalidatedPaths(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			want:     []finding{{".github/workflows/tests-e2e-upgrade.yaml", true}},
		},
		{
			...
			want:     []finding{{".github/workflows/auto-labeler.yaml", false}},
		},
		{
		...
		},
		{
			...
			want:     []finding{{".github/workflows/lint-go.yaml", true}},
		},
		{
...
func TestNoticeConverges(t *testing.T) {
	...
	added := append(slices.Clone(unvalidated),
		finding{".github/workflows/tests-e2e-upgrade.yaml", true})
