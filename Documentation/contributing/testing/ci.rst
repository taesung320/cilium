...
.. list-table:: Validation routes
   ...
   * - M5
     - Mirror the branch, temporarily add a ``pull_request`` trigger to the
       workflow, and open a test pull request. This is the one route that does
       need a pull request, since nothing else raises a ``pull_request`` event.
       Close it afterwards, and revert the trigger before merge.
...
Two rows deserve a warning. A change to an ``on:`` block is only partly covered
by M5, because GitHub evaluates triggers, ``types``, ``branches`` and ``paths``
filters when the real event fires: a temporary ``pull_request`` trigger says
nothing about ``pull_request_target`` semantics, a ``push`` filter or a cron. And
``.github/ariane-config.yaml`` cannot be exercised at all, since Ariane reads it
from the default branch once merged, so review is the only control there.
...
when it is. ``.github/ariane-config.yaml`` is read by Ariane itself and takes
